...
| 내보내기   | 이력서/포트폴리오 PDF 다운로드 (Puppeteer 생성)         |
...
이 파일을 고치면 **웹사이트는 즉시** 반영됩니다. (PDF는 미리 생성된 파일이라 수정 후 `pnpm pdf` 재생성이 필요합니다 — 아래 [내보내기](#-이력서--포트폴리오-내보내기-pdf-다운로드) 참고)

```ts
export const portfolioData = {
  personal: { name, title, email, summary, experienceSince, ... },
  experiences: [ ... ],     // 경력
  projects: [ ... ],        // 프로젝트
  skillCategories: [ ... ], // 기술
  activities: [ ... ],      // 대외활동
...
- **프로젝트 이미지**: `images`(스크린샷 갤러리)를 넣으면 웹에서 클릭 시 크게 볼 수 있습니다(라이트박스). 선택 항목입니다.
...
`main` 브랜치에 push 하면 [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml) 가 자동으로 빌드·배포합니다. Pages 소스는 **GitHub Actions** 로 설정되어 있습니다.

배포 주소: **https://gangjoohyeong.github.io/portfolio/**

> 레포명이 다르면 [`next.config.mjs`](next.config.mjs) 의 `REPO_BASE_PATH` 와 [`lib/site.ts`](lib/site.ts) 의 `repo`/`url` 을 바꾸세요.
